...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    await queryInterface.addColumn('PurchaseOrderProducts', 'profit', {
    ...
    await queryInterface.sequelize.query(`
      ...
      SET profit = (
        SELECT COALESCE(tp.lowest_fba_price, 0) - COALESCE(tp.fees, 0) - pop.unit_price
        FROM TrackedProducts AS tp
        WHERE tp.product_id = pop.product_id
      )
    `);
  },
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    await queryInterface.removeColumn('PurchaseOrderProducts', 'profit', {
